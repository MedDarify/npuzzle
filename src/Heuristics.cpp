...
bool Heuristics::isSolvable(const HeuristicArgs& args)
{
    int startInversions = countInversions(args.state);
    int goalInversions = countInversions(args.goalState);
    ...
    else
    {
        ...
        {
            if (args.state[i] == 0)
                startZeroRow = i / args.size;
            if (args.goalState[i] == 0)
                goalZeroRow = i / args.size;
        }
...
int Heuristics::calculateManhattan(const HeuristicArgs& args) 
{
    const std::vector<int>& state = args.state;
    const std::vector<int>& goal = args.goalState;
    int N = args.size;
...
int Heuristics::calculateHamming(const HeuristicArgs& args) 
{
    const std::vector<int>& state = args.state;
    const std::vector<int>& goal = args.goalState;
...
int Heuristics::calculateLinearConflict(const HeuristicArgs& args) 
{
    int h = calculateManhattan(args);
